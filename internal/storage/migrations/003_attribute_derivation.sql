-- ===========================================================
-- 003_attribute_derivation.sql
-- Public/Private Attribute Classification & Derivation Tracking
-- ===========================================================
...
CREATE TABLE IF NOT EXISTS kyc_attribute_derivations (
    ...
    derived_attribute_code TEXT NOT NULL REFERENCES kyc_attributes(code),
    source_attribute_code TEXT NOT NULL REFERENCES kyc_attributes(code),
    rule_expression TEXT NOT NULL,
    rule_type TEXT CHECK (rule_type IN ('Boolean', 'Numeric', 'String', 'Lookup')) DEFAULT 'Boolean',
    description TEXT,
...
COMMENT ON COLUMN kyc_attribute_derivations.rule_expression IS
    'DSL or formula describing transformation, e.g., "if value in [''IR'',''KP''] then true"';

COMMENT ON COLUMN kyc_attribute_derivations.rule_type IS
    'Classification of derivation: Boolean (flags), Numeric (scores), String (computed text), Lookup (reference data)';
